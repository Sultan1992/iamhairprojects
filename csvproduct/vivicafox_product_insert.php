...
if ($row9[cnt] == 0) {
	...
	$sql1 = "insert into cscart_products (product_id,product_code, status, amount, weight,
                 timestamp, tracking, min_qty, tax_ids, options_type, list_price) values 
     	     	($max_id,'$row[item_code]','H',100,1,unix_timestamp(now()),'B',1,6,'S',$row[price2])";
	...
	$t_full_06 = $row[featured_brand_name]." ".$row[hair_type];
	$t_full_06 = str_replace("Lace Front", "Lace Front Wig", $t_full_06);
	$t_full_09 = $row[hair_type];
	$t_full_09 = str_replace("Lace Front", "Lace Front Wig", $t_full_09);
	$t_full_12 = $row[featured_brand_name]." ".$row[hair_type]." COLLECTION";
	...
	$t_full_description = str_replace("##8", $row[item_name], $t_full_description);
	$t_full_description = str_replace("##9", $t_full_09, $t_full_description);
	$t_full_description = str_replace("##10", $row[fiber], $t_full_description);
	$t_full_description = str_replace("##11", $detail_description, $t_full_description);
