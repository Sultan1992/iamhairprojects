...
if ($row9[cnt] == 0) {
	...
	$sql1 = "insert into cscart_products (product_id,product_code, status, amount, weight,
                 timestamp, tracking, min_qty, tax_ids, options_type, list_price) values 
     	     	($max_id,'$row[item_code]','H',$Q,1,unix_timestamp(now()),'B',1,6,'S',$row[price2])";
	$result1 = mysql_query($sql1);

	$t_product = "New Born Free ".$row[hair_type]." - ".$row[item_name]." ".$row[featured_brand_name]." ".$row[fiber]." ".$row[hair_type];
	...
	$t_full_02 = "New Born Free ".$row[item_name]." ".$row[featured_brand_name]." ".$row[hair_type];
	$t_full_02 = str_replace("Lace Front", "Lace Front Wig", $t_full_02);

	//$t_full_05 = "New Born Free ".$row[featured_brand_name];
	$t_full_05 = "New Born Free ".$row[hair_type];

	$t_full_12 = "New Born Free ".$row[item_name]." ".$row[featured_brand_name]." ".$row[fiber]." ".$row[length]." length ".$row[hair_type];
	...
	$t_full_description = str_replace("##13", $detail_description, $t_full_description);
...
//	       5(Hair Length)
//	       6(Hair Types)
//	       8(Estimated Shipping Day)
//	       13(Manufacturer)
//--------------------------------------------------------------------
//$t_full_05 = str_replace("Chade", "New Born Free", $t_full_05);
...
$t_features_day_id = "5338";
call_variant_insert(157,$max_id,$t_features_day_id);

//$t_features_manufacturer_id = call_variant($t_brand);
$t_features_manufacturer_id = "5337";
